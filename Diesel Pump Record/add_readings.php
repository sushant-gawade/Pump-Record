...
$form_title = "⛽ Daily Nozzle Readings (Diesel)";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($new_dispensed < 0) {
        ...
        try {
            if ($id) {
                // --- UPDATE LOGIC (Editing Existing Reading by ID) ---
                $is_update = true;
                ...
                // Check if date already exists (to correctly calculate adjustment for ON DUPLICATE)
            ...
            $message = "<div class='alert alert-success'>✅ Readings $status. **" . number_format($new_dispensed, 2) . "L** diesel dispensed today. Available stock adjusted.</div>";
            ...
            $edit_id = $id; 
            $form_title = $edit_id ? "✏️ Edit Nozzle Reading (ID: $edit_id)" : "⛽ Daily Nozzle Readings (Diesel)";
        }
...
<div class="container mt-5">
    ...
    <form action="add_readings.php" method="POST">
        ...
        <div class="mb-3">
            ...
            <?php if ($edit_id): ?>
                <div class="form-text">Date cannot be changed during edit.</div>
            <?php endif; ?>
        </div>
