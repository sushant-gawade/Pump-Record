...
$form_title = "➕ Add New Diesel Stock";
...
if ($edit_id) {
    ...
    $form_title = "✏️ Edit Diesel Stock Entry (ID: $edit_id)";
    ...
    if ($new_liters <= 0) {
        ...
        try {
            if ($id) {
                ...
                $message_text = "Diesel stock entry updated successfully! Available stock adjusted by " . number_format($liters_difference, 2) . "L.";
                ...
                $message_text = "Diesel stock of " . number_format($new_liters, 2) . "L added successfully!";
            }
            ...
            if ($id) {
                 header("location: diesel_history.php");
                 exit;
            }
            
        } catch (Exception $e) {
            $conn->rollback();
            $message = "<div class='alert alert-danger'>❌ Error processing diesel stock: " . $e->getMessage() . "</div>";
...
<div class="container mt-5">
    <a href="diesel_history.php" class="btn btn-secondary mb-3">⬅️ Back to Stock History</a>
    ...
    <form action="add_stock.php" method="POST">
        ...
        <div class="mb-3">
            <label for="liters_added" class="form-label">Diesel Stock Added (Liters)</label>
